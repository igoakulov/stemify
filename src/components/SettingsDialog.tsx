...
export function SettingsDialog() {
  ...
  const [log_count, set_log_count] = useState(0);
  const [log_message, set_log_message] = useState("");
  ...
  useEffect(() => {
    ...
    const raf = window.requestAnimationFrame(() => {
      set_log_count(get_scene_log_count());
    });
  ...
  const on_export_logs = () => {
    export_scene_logs();
    set_log_message("Exported!");
    setTimeout(() => set_log_message(""), 1500);
  };
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="text-sm font-medium">
              Scene Log ({log_count} entries)
            </div>
            <div className="text-xs text-muted-foreground space-y-1">
              <div>
                Use as backup or send to developer to improve the Assistant.
              </div>
              <div>
                Does not save chats, only scene code generated by the Assistant.
              </div>
              <div>You can preview in any text editor (like Notepad).</div>
            </div>
            <div className="flex items-center justify-between pt-1">
              ...
              <Button type="button" size="sm" onClick={on_export_logs}>
                Export
              </Button>
            </div>
          </div>
